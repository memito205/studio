...
const TalladoMercanciaModule = dynamic(() => import('@/components/tallado-mercancia/TalladoMercanciaModule').then(mod => mod.TalladoMercanciaModule), { loading: () => <LoadingSpinner /> });
...
export const SuiteApp: React.FC<SuiteAppProps> = ({ theme = 'light' }) => {
  ...
  const getStandardizedName = useCallback(() => {
    if (!user) return 'Desconocido';
    const rawName = user.displayName || user.email || 'Desconocido';
    const standardized = PACKERS.find(p => p.toUpperCase() === rawName.toUpperCase());
    if (standardized) return standardized;
    return localStorage.getItem(`op_name_${user.uid}`) || rawName;
  }, [user]);
  ...
  const [referenceGoals, setReferenceGoals] = useState<ReferenceGoals>({});
  ...
  useEffect(() => {
    if (appStep === 'wholesale' || appStep === 'upload') {
      ...
      const fetchMappings = async () => {
        ...
        if (result.data) {
          setManualOperatorMappings(result.data);
        }
      };
      fetchMappings();
    }
  ...
  const handleNavigateToDashboardsLabeling = () => setAppStep('dashboards_labeling');
  const handleNavigateToDashboardsGastosTransporte = () => setAppStep('dashboards_gastos_transporte');
  ...
  const handleStartPacking = async (order: WholesaleOrder) => {
      ...
      const initialSession: PackingSession = sessionResult.data || {
          orderId: order.id,
          packerId: user.uid,
          packerName: getStandardizedName(),
          units: [],
          status: 'active',
          pauses: [],
      };
  ...
  const handleManualOperatorMappingChange = (newMappings: ManualOperatorMappings) => { setManualOperatorMappings(newMappings); };
  ...
  const renderContent = () => {
      switch(appStep) {
          ...
          case 'dashboards_gastos_transporte': return <GastosTransporte onReturn={() => setAppStep('dashboards_bodega')} />;
