...
import { Archive, Building, ShoppingBag, Truck, Settings, Tags, PackagePlus, Calculator, FileBarChart, Printer } from 'lucide-react';
...
const ModuleCard: React.FC<ModuleCardProps> = ({ icon: Icon, title, description, actionText, onAction, disabled }) => {
    return (
        <Card className="flex flex-col text-center items-center transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
...
export const SuiteDashboard: React.FC<SuiteDashboardProps> = ({ onNavigateToPackingModule, onNavigateToWholesaleModule, onNavigateToLogisticsModule, onNavigateToGeneralSettings, onNavigateToLabelControlModule, onNavigateToMerchandiseLabelingModule, onNavigateToBagDistributionModule, onNavigateToMerchandiseReceptionModule, onNavigateToOtherFeaturesModule }) => {
    const { role } = useAuth();
    ...
    const modules = [
        ...
        },
        {
            ...
            icon: ShoppingBag,
            title: "Compras Bolsas y Distribución",
            description: "Gestione las compras de bolsas para tiendas y su distribución.",
            actionText: "Acceder",
        ...
        },
        {
            ...
            description: "Herramientas adicionales como la calculadora de costo financiero.",
            actionText: "Acceder",
