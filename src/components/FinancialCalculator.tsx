...
import { calculateAmortization, CreditCalculationResult, GeneralSummary, parseDateString, convertAnnualToMonthlyRate } from "@/services/creditCalculations";
...
import ExportGerencialPdfButton from "@/components/financial-calculator/ExportGerencialPdfButton";
...
export const FinancialCalculator: React.FC<FinancialCalculatorProps> = ({ onReturn }) => {
  ...
  const processData = (data: any[], currentMonthlyGraceRates: Record<string, number>) => {
    ...
    data.forEach((row, index) => {
      ...
      if (typeof row.fecha === 'number') {
        ...
        fechaCredito = `${date.y}-${String(date.m).padStart(2, '0')}-${String(date.d).padStart(2, '0')}`;
      } else {
      ...
      const isValidRow = 
        !isNaN(valorCredito) && valorCredito >= 0 &&
        modalidadPago && typeof modalidadPago === 'string' && modalidadPago.trim() !== '' &&
        !isNaN(numCuotas) && numCuotas > 0 &&
        !isNaN(tasaInteres) && tasaInteres >= 0 &&
        !isNaN(vrAdmon) && vrAdmon >= 0 &&
        !isNaN(ivaAdmon) && ivaAdmon >= 0 &&
        fechaCredito && typeof fechaCredito === 'string' && fechaCredito.trim() !== '' &&
        puntoDeVenta !== '' && puntoDeVenta.toLowerCase() !== 'null' && puntoDeVenta.toLowerCase() !== 'undefined' &&
        documento !== '' && documento.toLowerCase() !== 'null' && documento.toLowerCase() !== 'undefined';
    ...
    showSuccess(`Procesamiento completado. ${totalCredits} créditos procesados.`);
    setIsLoading(false);
  };
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-7xl">
       {onReturn && (
         <Button onClick={onReturn} variant="outline" className="mb-4">
      ...
      <div className="mb-6 p-4 border rounded-lg shadow-sm bg-gray-50 dark:bg-gray-800">
        ...
        <Button 
          ...
          disabled={isLoading || !excelRawData} 
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <ExportGerencialPdfButton summaryExists={!!generalSummary && generalSummary.totalCredits > 0} />
      </div>
